 ...
 * целиком.
 *
 * Критерий один: в каталоге нет продуктовой семантики — ни `@/api`, ни
 * `@/store`, ни доменных типов `@/types`. Это библиотеки формы и картинок, а не
 * интерфейс экспертного режима. Держать свою копию каждой иконки значило бы
 * разъехаться с апстримом в мелочах ради нулевой выгоды — тот же довод, по
 * которому общими сделаны примитивы.
 *
 * ⚠️ Критерий сторожит `src/simple/openDirsPurity.test.ts` (#37) на каждом
 * `npm test`, а не разовая проверка руками. Список внешних зависимостей здесь
 * когда-то был перечислен поимённо «проверено 14.08.2026» — и к #51 успел
 * соврать: `components/ui` с тех пор тянет ещё `react-router` (`BentoCard.tsx`),
 * `react-i18next` (`Spinner.tsx`) и `react-dom` (`Sheet.tsx`). Апстримные
 * зависимости меняются каждый синк, так что состав здесь не перечисляем вообще:
 * запрет на api/store/types проверяется, а не документируется.
 *
 * ⚠️ `components/primitives` открыт тем же правилом, но не этим списком — он
 * исключён отдельными условиями ниже. Состав открытых каталогов сверяется со
 * списком сторожа чистоты (там же, `openDirsPurity.test.ts`): добавленный сюда
 * четвёртый каталог иначе остался бы без проверки на чистоту молча.
 *
