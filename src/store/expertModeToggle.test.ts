...
describe('переключатель режима в апстримной шапке', () => {
  ...
  it.each(HEADERS)('$name: стор режима берётся из @/store/mode', ({ file }) => {
    ...
    // апстримным файлам импортировать презентацию простого режима.
  ...
  it.each(HEADERS)('$name: подпись — ключ из нашего неймспейса (#46)', ({ file }) => {
    ...
    // (гейт границ запрещает импорты, но не значения), поэтому неймспейс
    // написан литералом. Ключ живёт в src/simple/locales/{ru,en}.json —
    // апстримные ru/en/fa/zh.json не трогаем: четыре файла, конфликтующих на
    // каждом синке ради одной строки.
  ...
  it.each(HEADERS)('$name: путь без простой страницы уводит на / (#45)', ({ file }) => {
    ...
    // узле кнопки. Мутация `=== null` → `!== null` не убирает ни один из вызовов
    // (`resolveSimpleRoute(` и `navigate('/')` в блоке остаются), поэтому старая
    // версия проверки — `toContain` по каждому куску отдельно — оставалась
    // зелёной, а поведение при этом переворачивалось обратно в дефект #45: на
    // пути без простой страницы клик ничего не делал, а на пути с ней — уводил
    // на `/` без нужды. Поэтому ищем условие целиком строкой и отдельно
    ...
    const condition = 'resolveSimpleRoute(location.pathname) === null';
  ...
  it.each(HEADERS)('$name: наличие простой страницы спрашивается у реестра', ({ file }) => {
    // Второго источника истины быть не должно: тем же resolveSimpleRoute
    // пользуется слой подмены в App.tsx. Свой матчинг путей в шапке разъехался
    // бы с реестром молча.
    expect(read(file)).toContain("from '@/simple/routes'");
  });

  it('src/simple/routes.tsx: resolveSimpleRoute экспортируется', () => {
    // Шапки зовут его по имени; переименование в реестре обязано ронять этот
    // тест, а не сборку в чужом MR.
  ...
  it("src/simple/i18n.ts: неймспейс объявлен строкой 'simple'", () => {
    // Шапки пишут ns: 'simple' литералом (импорт SIMPLE_NS из src/simple/ им
    // запрещает гейт границ). Если константу NS в src/simple/i18n.ts
    // переименуют или изменят значение, литерал в шапке молча разойдётся с
    // реальным неймспейсом, и подпись отрисуется как сырой ключ. Проверяем факт
    // объявления, а не импортируем модуль: environment у vitest — 'node',
    // импорт потянул бы i18next и весь граф.
