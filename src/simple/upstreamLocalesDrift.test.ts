...
/**
 * Плоская карта «точечный путь → текст» по всем листьям дерева локали.
 *
 * Нужна, чтобы пересечение неймспейсов СЧИТАЛОСЬ, а не перечислялось руками:
 * захардкоженный список одноимённых ключей — это обещание покрытия, которое
 * протухает молча при следующем таком ключе.
 */
function leaves(tree: Tree, prefix = '', out: Record<string, string> = {}) {
  for (const [key, value] of Object.entries(tree)) {
    const path = prefix ? `${prefix}.${key}` : key;
    if (typeof value === 'string') out[path] = value;
    else if (value !== null && typeof value === 'object') leaves(value, path, out);
  }
  return out;
}

const RU_UPSTREAM_LEAVES = leaves(JSON.parse(readFileSync('src/locales/ru.json', 'utf8')) as Tree);
const RU_SIMPLE_LEAVES = leaves(ru as Tree);

/** Ключи, живущие в ОБОИХ неймспейсах. Вычисляются, а не перечисляются. */
const SHARED_KEYS = Object.keys(RU_SIMPLE_LEAVES).filter((key) => key in RU_UPSTREAM_LEAVES);

/**
 * Одноимённые ключи, у которых совпадение текстов ПОКА допущено.
 *
 * ⚠️ Это не «разрешено», а «известно и вынесено в отдельный разбор». У обоих
 * текст в `src/simple/locales/ru.json` дословно повторяет апстримный — то самое
 * дублирование апстримной строки, которое канон (`docs/architecture/two-modes.md`,
 * раздел «Локали») называет признаком сломанной развязки неймспейсов. Значит,
 * либо копия в `simple` не нужна вовсе, либо дизайн-правка до неё не доехала;
 * какой исход верный — решение владельца, а не догадка сторожа. Разбор заведён
 * отдельной задачей **#77**; к #75 (подпись пункта меню) он отношения не имеет,
 * и чинить его попутно было бы расширением скоупа.
 *
 * Новый одноимённый ключ под эту поблажку НЕ попадает: список ниже считается по
 * пересечению локалей, поэтому очередной ключ с совпавшим текстом красит сторож
 * сам, без правки каких-либо списков здесь.
 */
const KNOWN_SAME = ['subscription.cta.expiredHint', 'subscription.cta.trialHint'];
...
describe('переехавшие строки живут в нашем неймспейсе (задача #33)', () => {
  ...
  it('разбор удался — пересечение неймспейсов посчитано и не пусто', () => {
    // Пара к сторожу ниже: он сравнивает тексты ТОЛЬКО по пересечению. Битый
    // путь к апстримной локали или пустое дерево дали бы пустой список пар, и
    // сравнивать стало бы нечего — сторож остался бы вечно зелёным.
    expect(Object.keys(RU_UPSTREAM_LEAVES).length, 'src/locales/ru.json').toBeGreaterThan(100);
    expect(Object.keys(RU_SIMPLE_LEAVES).length, 'src/simple/locales/ru.json').toBeGreaterThan(10);
    expect(SHARED_KEYS.length, SHARED_KEYS.join(', ')).toBeGreaterThanOrEqual(7);
  });

  it('у одноимённых ключей тексты РАЗНЫЕ — иначе переезд был бы бессмысленным', () => {
    // Ключи живут в обоих неймспейсах ровно ради разных слов. Совпади тексты —
    // копия в `simple` стала бы дублированием апстримной строки, то есть тем
    // самым, что канон запрещает; а привести их к апстримным словам значило бы
    // молча откатить дизайн-правку простого экрана.
    //
    // ⚠️ Пары берутся ПЕРЕСЕЧЕНИЕМ листовых ключей `src/locales/ru.json` и
    // `src/simple/locales/ru.json`, а не списком: список одноимённых ключей —
    // это обещание покрытия, и оно протухает молча, стоит завести очередной
    // такой ключ. Отсюда же следует, что новый одноимённый ключ проверяется сам,
    // без правок здесь. Пример из свежих: `news.title` (#75) — не переезд
    // существующей строки, а НОВЫЙ ключ в `simple` под апстримным именем, но
    // требование к нему то же: тексты обязаны различаться, иначе пункт меню и
    // заголовок `/news` молча возвращаются к «Новости и обновления».
    const same = SHARED_KEYS.filter(
      (key) => RU_SIMPLE_LEAVES[key] === RU_UPSTREAM_LEAVES[key] && !KNOWN_SAME.includes(key),
    );

    expect(same).toEqual([]);

    // Обратная сторона поблажки: как только #77 разведёт тексты, ключ обязан
    // уйти из `KNOWN_SAME`. Иначе исключение переживёт свою причину и тихо
    // прикроет следующее совпадение на том же ключе.
    const stale = KNOWN_SAME.filter((key) => RU_SIMPLE_LEAVES[key] !== RU_UPSTREAM_LEAVES[key]);

    expect(stale, 'тексты разведены — убери ключ из KNOWN_SAME (#77)').toEqual([]);
  });
