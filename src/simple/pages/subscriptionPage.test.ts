...
describe('истёкшая подписка: одно действие на экране (#65)', () => {
  ...
  it('ради кнопки истёкшей подписки страница баланс больше НЕ запрашивает (#70)', () => {
    // ⚠️ Запрос жил здесь только ради двух снятых требований: грубого порога
    // «денег хватает» и заглушки «баланс ещё грузится». Оба ушли вместе с
    // решением по остатку — кнопка ведёт на экран оплаты, где цена известна.
    // Оставленный запрос был бы мёртвым кодом, а сторож при нём — зелёным на
    // пустоте.
    expect(code).not.toContain('useBalanceQuery');
    expect(code).not.toContain('balanceData');
    // Своего запроса баланса на странице нет и не появилось: копия этих четырёх
    // строк и была бы началом расхождения.
    expect(code).not.toContain('balanceApi.getBalance');
    // Пара «разбор удался»: сам общий запрос никуда не делся — им пользуются
    // главная, экран оплаты и страница баланса.
    expect(read('src/simple/components/BalanceWidget.tsx')).toContain(
  ...
  it('связка «ключ + queryFn» записана в общем запросе, и одной строки для неё мало', () => {
    ...
    // стоять рядом в одном объявлении. Страница инвалидирует именно `['balance']`
    // после паузы списаний — разъедься ключ, и виджет баланса остался бы со
    // старой суммой, молча и при зелёной сборке.
