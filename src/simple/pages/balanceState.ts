...
// тест этого модуля на разрешении модулей. Сам `utils/paymentStatus` не
 ...
 * alias `@/`, а он в тестах не разрешается (канон, «Тесты»), и
 * без такого модуля ветки «во что превратился код ошибки промокода», «как
 * подписать операцию в истории», «куда уводит возврат от шлюза» остались бы
 * непроверенными вообще.
 *
 * ⚠️ Правил блока способов оплаты здесь БОЛЬШЕ НЕТ (#58): `resolveAccentedMethodId`
 * и `topUpHref` уехали в `src/simple/components/paymentMethodsState.ts`, рядом с
 * их единственным потребителем — компонентом `PaymentMethodsGrid`. Тот импортировал
 * их отсюда, то есть слой компонентов зависел от состояния страницы, названной по
 * одному из двух своих экранов.
 *
