...
describe('карточка FAQ живёт в копии (задача #63)', () => {
  ...
  it('карточка стоит ПЕРЕД формой обращения, как на сегодняшнем стенде', () => {
    ...
    const form = page.indexOf("tSimple('support.yourTickets')");
  ...
  it('всё остальное на странице продолжает читаться из апстримного неймспейса', () => {
    // Спека: в наш неймспейс уезжают ТОЛЬКО ключи наших патчей, поимённо.
    // Уехавшая следом апстримная строка означала бы, что мы начали дублировать
    // локали, — а это ровно та беда, из-за которой заведён отдельный неймспейс.
    //
    // Список рос один раз и по делу: два ключа карточки FAQ (#63) плюс три
    // заголовка, которые форк переформулировал прямо в `src/locales/ru.json`
    // (#33). Пока те три жили в апстримном файле, откатить его к `upstream/main`
    // было нельзя: экспертный `src/pages/Support.tsx` читает те же ключи, и
    // откат менял бы слова простому экрану, а сохранение — навязывало наши
    // слова экспертному.
    expect(page).toContain('const { t } = useTranslation();');

    const ourKeys = [...page.matchAll(/tSimple\('([^']+)'/g)].map((found) => found[1]);

    expect(ourKeys.sort()).toEqual([
      'support.faqLinkHint',
      'support.faqLinkTitle',
      'support.newTicket',
