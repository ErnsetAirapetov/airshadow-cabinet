...
describe('виджет баланса — один компонент на два экрана (задача #53)', () => {
  ...
  it('тон по умолчанию — плоский, а кегль общий с главной (#72, #73)', () => {
    // ⚠️ Критерий приёмки #72: акцентная ЗАЛИВКА принадлежит одной главной.
    // Держится это ИСКЛЮЧИТЕЛЬНО на значении по умолчанию: оба экрана зовут
    // `<BalanceWidget />` без пропов, и замена дефолта на `'accent'` перекрасила
    // бы их (а заодно экран оплаты подписки) при зелёных тестах — вызовы-то
    // запинены выше, а дефолт не сторожил никто.
    expect(widget).toContain("tone = 'flat'");
    // ⚠️ А вот КЕГЛЬ с #73 общий: плоская пара `text-4xl sm:text-5xl` отсюда
    // ушла, потому что из-за неё сумма меняла размер при переходе главная →
    // `/balance`. Тон и размер — разные вещи, и совпадать обязан именно размер.
    // Состав правила сторожит `balanceCardParity.test.ts`.
    expect(widget).not.toContain("'text-4xl font-bold text-dark-50 sm:text-5xl'");
    expect(widget).toContain('resolveBalanceValueClass(formatted)');
  });
