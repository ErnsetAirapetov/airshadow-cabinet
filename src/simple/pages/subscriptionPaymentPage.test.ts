...
describe('оплата подписки живёт на ОДНОМ экране (задача #69)', () => {
  it('прежней отдельной страницы продления больше нет', () => {
    ...
    // ⚠️ С #70 владелец экрана продления РОВНО ОДИН. Раньше рядом стоял блок
    // действия истёкшей подписки: он продлевал сам, решая по грубому порогу
    // «есть ли хоть рубль», и обещал продление, которого не будет. Теперь он
    // ведёт СЮДА — на экран, где цены известны и где нехватку называют цифрой.
    expect(owners.sort()).toEqual([PAGE]);
  });
