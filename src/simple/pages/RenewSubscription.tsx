...
/**
 * Продление подписки в простом режиме (задача #29).
 *
 * ⚠️ Это НЕ новый интерфейс, а копия апстримного
 * `src/pages/RenewSubscription.tsx` с одним изменением по спеке владельца от
 * 18.08.2026: инлайновый блок баланса заменён общим `BalanceWidget` — тем же,
 * что на балансе и на экране суммы пополнения. Причина названа владельцем
 * дословно: «в текущем виде баланс теряется». Оригинал остаётся экспертному
 * режиму нетронутым.
 *
 * ⚠️ Ловушка двух источников баланса закрыта. Апстрим брал баланс из
 * `purchaseOptions.balance_kopeks` (`['purchase-options', subId]`), а виджет
 * спрашивает свой `['balance']`. Оставь оба — виджет показывал бы одну сумму, а
 * кнопка периода писала бы «не хватает» по другой. Поэтому запрос
 * `purchase-options` со страницы убран совсем (он существовал ровно ради
 * баланса), число берётся из `resolveRenewBalance` и уходит ДВУМЯ путями из
 * одного значения: в виджет пропом `balanceRubles` и в расчёт доступности как
 * `kopeks`. Разбор — в докстринге `./renewState`.
 */
export function SimpleRenewSubscription() {
