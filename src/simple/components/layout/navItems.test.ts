...
describe('состав пунктов простого режима', () => {
  ...
  it('список — ровно шесть пунктов, назначенных владельцем', () => {
    // Сторож состава: сверка полная, а не «содержит». Добавление или удаление
    // пункта обязано быть осознанным и видным в дифе.
...
describe('нижнее меню — подмножество общего списка', () => {
  ...
  it('признак стоит ровно у четырёх пунктов — профиль и информация только в меню', () => {
