...
// ──────────────────────────────────────────────────────────────────
// TariffPickerGrid — витрина тарифов простого режима (задача #29)
//
// ⚠️ Это КОПИЯ апстримного
// `src/components/subscription/purchase/TariffPickerGrid.tsx` с ровно
// одним изъятием по спеке владельца от 18.08.2026: убран виджет
// промо-группы («Ваша группа: …» + «персональные скидки применены»),
// который в апстриме занимал первый экран. Оригинал остаётся
// экспертному режиму нетронутым — так работает канон
// docs/architecture/two-modes.md.
//
// ⚠️ Скидки промо-группы при этом НИКУДА НЕ ДЕЛИСЬ: убран только
// баннер. Цены по-прежнему считает `usePromoDiscount`, а бейдж
// `isPromoGroup` красит проценты зелёным — то есть человек видит свою
// скидку в деньгах, а не в объявлении про неё.
//
// Renders:
...
export function TariffPickerGrid({
  ...
  return (
    <>
      {/* Виджет промо-группы убран (спека владельца, задача #29): он занимал верх
          экрана ради строчки «Ваша группа», которую человек и так видит скидкой в
          ценах ниже. Апстримный блок стоял ровно здесь — см. оригинал
