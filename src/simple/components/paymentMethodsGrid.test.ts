 ...
 *
 * ⚠️ Разметка читается ТЕКСТОМ, а не импортируется. Компонентных тестов в проекте
 ...
 * компонента потянул бы весь граф приложения и упал бы на разрешении модулей. Тот
...
describe('акцентная карточка способа пополнения (задачи #30, #52, #55)', () => {
  ...
  it('акцент достаётся буквально первой карточке, а решает это чистая функция', () => {
    ...
    // `balanceState.test.ts`; здесь сторожим, что компонент его ЗОВЁТ, а не
    // переписал условие в разметке.
