...
/**
 * Копия апстримного `src/components/tickets/MessageMediaGrid.tsx` (задача #63).
 *
 * Гейт границ не пускает простой режим в `src/components/tickets/**`, а галерея
 * вложений — часть переписки в тикете, без неё страница поддержки неполна.
 * Скопировано КАК ЕСТЬ: спека владельца на эту задачу — «перенести в том виде,
 * что есть сейчас», упрощение сюда не входит. Отличие от оригинала ровно одно —
 * путь импорта `ticketsApi` через alias.
 *
 * ⚠️ Копия живая, апстрим правит оригинал каждый синк. Расхождение сторожит
 * `src/simple/components/supportCopies.test.ts` — он сверяет файлы целиком после
 * нормализации путей импорта и комментариев. Снимать сторож можно только вместе
 * с копией.
 */
