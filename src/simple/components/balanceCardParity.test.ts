...
describe('кликабельность карточки на главной (задача #73)', () => {
  ...
  it('карточка не отзывается на наведение НИ ПРИ КАКОМ значении пропа (задача #79)', () => {
    // ⚠️ Решение владельца: реакции на курсор у этой карточки нет вовсе.
    // Остальные элементы простого режима на наведение не отвечают, и уникальное
    // поведение у одного виджета читается случайностью, а не подсказкой; на
    // сплошной акцентной заливке в светлой теме белый spotlight
    // `.bento-card-hover::after` вдобавок бьёт в глаза вспышкой.
    //
    // ⚠️ Запрещён САМ КЛАСС, а не конкретный сегодняшний тернарник. Hover сюда
    // уже добавляли однажды (#73, по замечанию ревью «единственный кликабельный
    // элемент главной без реакции на курсор») и попробуют вернуть по той же
    // логике — проверка обязана краснеть на любом его возврате, в том числе
    // безусловном.
    expect(widget).not.toContain('bento-card-hover');
    // Парная половина: хром `bento-card` карточка при этом не теряет — «нет
    // hover» не должно однажды превратиться в «нет поверхности вовсе».
    expect(widget).toContain("isAccent ? 'bento-card h-full' : 'bento-card'");
  });

  it('шеврон включает только главная', () => {
