...
const dashboardSource = readFileSync('src/simple/pages/Dashboard.tsx', 'utf8');
...
describe('состав сквозного списка', () => {
  ...
  it('колеса в списке нет — из простого режима туда не ведёт ни одна кнопка', () => {
    // Отдельным сторожем, а не только полной сверкой выше: решение владельца
    // «колесо в простом режиме не используем вовсе» (#66) читается здесь
    // словами, а не вычитается из отсутствия строки в длинном списке.
    expect(PASSTHROUGH_PATHS).not.toContain('/wheel');
  });
...
/**
 * Посылка, на которой держится удаление `/wheel` из сквозного списка (#66).
 *
 * Сквозной список существует ровно для адресов, куда ведут НАШИ кнопки: он
 * оставляет человека в простой оболочке там, где своей простой страницы нет.
 * Владелец решил, что колесо фортуны в простом режиме не используется вовсе, —
 * баннер с простой главной убран, и на `/wheel` из простого режима не ведёт ни
 * одна кнопка. Держать путь в списке после этого незачем.
 *
 * ⚠️ Сторож стоит здесь, а не отдельным файлом, именно потому что это ПОСЫЛКА
 * решения, а не независимый факт: вернётся баннер — вернуть надо и запись, иначе
 * кнопка снова станет выходом из простого режима. Красное здесь и есть напоминание.
 *
 * Прямая ссылка `/wheel` при этом продолжает работать: адрес открывается
 * апстримной страницей в апстримной оболочке, как любой путь вне обоих списков.
 * Решение обратимо одной строкой в каждом месте.
 */
describe('колеса в простом режиме нет вовсе', () => {
  const dashboardCode = stripComments(dashboardSource);

  it('разбор удался — простая главная прочитана', () => {
    // Без пары переименование файла дало бы вечно зелёные проверки ниже.
    expect(dashboardSource.length).toBeGreaterThan(1000);
    expect(dashboardCode).toContain('export function SimpleDashboard()');
  });

  it('на простой главной нет ссылки на колесо', () => {
    expect(dashboardCode).not.toContain('/wheel');
  });

  it('простая главная не спрашивает у бэкенда конфиг колеса', () => {
    // Запрос жил ровно ради баннера (`wheelConfig?.is_enabled`). Остался бы —
    // это лишний поход в сеть на LCP-экране ради невидимого блока.
    expect(dashboardCode).not.toContain('wheelApi');
    expect(dashboardCode).not.toContain('wheel-config');
  });

  it('подписей баннера на простой главной тоже нет', () => {
    expect(dashboardCode).not.toContain('wheel.banner');
  });
});
...
describe('точка входа отдаёт наружу обе новые функции', () => {
  ...
  it('hasSimpleView складывает реестр СО сквозным списком', () => {
    ...
    // одиннадцати сквозных путях, и переключение режима на `/profile` уводит на
