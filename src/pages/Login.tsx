      ...
      <div className="relative w-full max-w-md space-y-5">
        ...
        {registeredEmail ? (
          ...
          <div className="card">
            ...
            {/* Telegram Mini App auto-auth status — рендерится только внутри
                Telegram WebApp. В браузере схлопывается в ничего, поэтому
                первым виден email-блок. */}
            {isTelegramWebApp && (
            ...
            {/* Email auth — ОСНОВНОЙ путь, всегда раскрыт */}
            {isEmailAuthEnabled && (
            ...
            {/* Второстепенное: OAuth-провайдеры + Telegram (браузер) */}
            {(oauthProviders.length > 0 || !isTelegramWebApp) && (
              <>
                ...
                {/* Telegram — неприметно, в самом низу. Существующий one-click
                    компонент, просто перенесён. Только в браузере: в Mini App
                    Telegram покрыт блоком авто-авторизации выше. */}
                {!isTelegramWebApp && (
