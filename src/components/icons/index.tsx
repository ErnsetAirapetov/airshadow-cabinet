...
//
// ⚠️ Единственная наша строка в этом апстримном файле, пережившая откат #33, и
// пережившая осознанно: иконку берёт НАШ слой —
// `src/simple/components/layout/navIcons.ts` рисует ею пункт «Поддержка» в
// меню простого режима. Правило канона «откат делается только тогда, когда
// смысл правки перенесён» здесь читается буквально: смысл никуда не перенесён,
// он тут и живёт. Слепой откат уронил бы сборку простого режима.
//
// Из экспертного режима SupportIcon этой же задачей убран — `AppShell`,
// `AppHeader`, `MobileBottomNav` и `CommandPalette` вернулись к апстримному
// `ChatIcon`. То есть иконка объявлена здесь, но потребитель у неё ровно один.
// `components/icons` — открытый каталог (канон, «Открытые каталоги»), простому
// режиму ходить сюда разрешено, поэтому копии в `src/simple/**` не заводим.
